  ...
  if (! initialized)
    {
      ...
      tmppath = concat (ld_sysroot, "/usr/src/output/host/etc/ld.so.conf",
			(const char *) NULL);
...
OUTPUT_ARCH(arm)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"=/usr/src/output/host/arm-buildroot-linux-uclibcgnueabihf/lib\"); SEARCH_DIR(\"=/usr/local/lib\"); SEARCH_DIR(\"=/lib\"); SEARCH_DIR(\"=/usr/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
