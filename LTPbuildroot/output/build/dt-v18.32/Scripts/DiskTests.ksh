...
typeset DT_NOPROGT=${DT_NOPROGT-"alarm=3s noprogt=15s notime=fsync"} # add "noprogtt=125s trigger=cmd:script"
typeset DT_PROCS=${DT_PROCS:-10}
...
#
typeset -i tci=0
for tc in SequentialIO ReverseIO RandomIO VariableIO IncrementalIO BufferModes
do
    TestCases[$tci]=$tc
    (( tci += 1 ))
done
